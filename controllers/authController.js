...
exports.sendVerificationCode = async (req, res) => {
	...
	try {
		...
		const codeValue = Math.floor(Math.random() * 1000000).toString();  //random code generated, crypto could also be used for this
		...
		if (info.accepted[0] === existingUser.email) {
			...
			existingUser.verificationCode = hashedCodeValue;  //stores hashed verification code, along with time in database
...
exports.verifyVerificationCode = async (req, res) => {
	...
	try {
		...
		if (Date.now() - existingUser.verificationCodeValidation > 5 * 60 * 1000) {  //in case user doesnt repond in 5 mins
			return res
				.status(400)
